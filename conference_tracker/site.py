...
// Build Google Calendar / Outlook "add event" links for a conference's dates.
function calendarLinks(row){
  const s = parseDay(row.start_date); if (!s) return null;
  const e = parseDay(row.end_date) || s;
  const endEx = addDays(e, 1);              // calendars treat the end as exclusive
  const text = encodeURIComponent(row.name || "");
  const loc  = encodeURIComponent(row.location || "");
  const det  = encodeURIComponent(isUrl(row.contact) ? row.contact : "");
  const google = "https://calendar.google.com/calendar/render?action=TEMPLATE"
    + "&text=" + text + "&dates=" + ymd(s) + "/" + ymd(endEx)
    + "&location=" + loc + "&details=" + det;
  const outlook = "https://outlook.office.com/calendar/0/deeplink/compose?path=/calendar/action/compose"
    + "&rru=addevent&subject=" + text + "&startdt=" + iso(s) + "&enddt=" + iso(endEx)
